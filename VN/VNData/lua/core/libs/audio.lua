function Audio()
    ...
    function Meta:Seek(pos) Bridge.Audio_Seek(self.id, pos) end
    function Meta:Pos() return Bridge.Audio_Pos(self.id) end

    function Meta:Volume(vol)
        if vol == nil then return self.volume end
        self.volume = vol
    ...
    function Meta:isPlaying() return Bridge.Audio_isPlaying() end
